...
- (void)_initialisation {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleDisableGridViewNotification:)
                                                 name:@"DISABLE_GRIDVIEW"
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleEnableGridViewNotification:)
                                                 name:@"ENABLE_GRIDVIEW"
                                               object:nil];
...
-(void)scrollToBottom {
    if (_gridController != nil) {
        CGPoint bottomOffset = CGPointMake(0, _gridController.collectionView.contentSize.height - _gridController.collectionView.bounds.size.height);
        [_gridController.collectionView setContentOffset:bottomOffset animated:NO];
    }
}

-(void)handleEnableGridViewNotification:(NSNotification *)notification {
    [self enableGridView:YES];
}

-(void)handleDisableGridViewNotification:(NSNotification *)notification {
    [self enableGridView:NO];
}
...
//    }
    ...
    UIImage * speakImage = [UIImage imageNamed:@"Dzst_function_icon-yy_normal@2x.png"];
    UIImage * speakImagePressed = [UIImage imageNamed:@"Dzst_function_icon-yy_press@2x.png"];
    UIImage * backImage = [UIImage imageNamed:@"Dzst_function_foot-bg@2x.png"];
    CGSize size = [backImage size];
    CGRect rect;
    CGRect bound = self.view.bounds;
    rect.size = size;
    rect.origin.x = 0;
    rect.origin.y = bound.size.height - size.height;
    ...
    _speakButton.frame = CGRectMake((bound.size.width - size.width) / 2, 0, size.width, size.height);
    [_speakButton setBackgroundImage:speakImage forState:UIControlStateNormal];
    
    [_speakButton addTarget:self action:@selector(speakButtonPressed:) forControlEvents:UIControlEventTouchDown];
    [_speakButton addTarget:self action:@selector(speakButtonReleased:) forControlEvents:UIControlEventTouchUpInside];
    [controlView addSubview:_speakButton];
    
    pressedCircle = [[UIImageView alloc] initWithImage:speakImagePressed];
    pressedCircle.frame = CGRectMake((bound.size.width - size.width) / 2, 0, size.width, size.height);
    pressedCircle.hidden = YES;
    [controlView addSubview:pressedCircle];
    
    CGRect frame2 = CGRectMake(bound.size.width - 140, bound.size.height - 45, 120, 40);
    fromLabel = [[UILabel alloc]initWithFrame:frame2];
    fromLabel.text = @"Group Tag";
    fromLabel.textColor = [UIColor whiteColor];
    //[controlView addSubview:fromLabel];
...
-(void) enableGridView:(BOOL)enable{
...
- (void)changeSwitch:(id)sender{
    ...
    self.displaySelectionButtons = [sender isOn];
    _gridController.selectionMode = [sender isOn];
...
-(void)speakButtonPressed:(UIButton *)sender{
    pressedCircle.hidden = NO;
    [self runSpinAnimationOnView:pressedCircle duration:1 rotations:1 repeat:1000];
    [self startRecord];
}

-(void)speakButtonReleased:(UIButton *)sender{
    pressedCircle.hidden = YES;
    [pressedCircle.layer removeAllAnimations];
    if (_gridController == nil) {
...
- (void)showGrid:(BOOL)animated {
    ...
    rrr = _gridController.view.bounds;
